...
CLI subcommands: `zipgo serve` (default), `zipgo deploy` (rsync a local dir to a remote zipgo host over SSH, creating the trailing-dot subdomain folder tree, and snapshotting the site's prior content into `.zipgo-versions/` first — see `internal/deploy`), `zipgo ls`/`zipgo info` (read-only SSH inspection of what's deployed on the remote target, `--json` for machine-readable output — see `internal/remote`), `zipgo rollback` (swap a past `.zipgo-versions/` snapshot back into place, `--list` shows the history — see `internal/remote/rollback.go`), `zipgo doctor` (lint the local domains folder — see `internal/doctor`), `zipgo enable|disable|status` (systemd user service), `zipgo help`.

### Remote inspection (`internal/remote`)
`ls`/`info` shell out to `ssh` (GNU coreutils assumed on the remote, as the existing `find -printf`/`date -r` calls already did). `siteDirs` finds every folder under the base and keeps the ones `deploy.HostFromRemote` maps back to a host; `stat` then runs **one** generated script (`statScript`) over exactly those folders, printing a tab-separated record per site that `parseSites` (pure, table-tested) turns into a `Site`. `Site` is the single source of truth for both the human output and `--json`: `Type` (`static`/`spa`/`proxy`/`redirect`), `Enabled` and `Protected` are recomputed the way `internal/builder` routes the site — SPA = `index.html` + a bundle dir, proxy = a `.zipgoconfig.json` `rewrite`, redirect = a `redirect`, protected = a `basicAuth` (reported as a bool; the hashes are never emitted) — so keep `bundleDirs` in sync with `internal/sites`, and an unreadable config surfaces as `ConfigError` rather than being dropped. `Size`/`Files` **prune nested trailing-dot folders and the `.zipgo-versions` history folder** (both are not the site's own current content), so an apex is not the sum of its whole domain nor of its backups. `--json` shapes: `ls` → `[]Site`, `ls <host>` → `[]Entry` (that folder's children), `info <host>` → one `Site`.

### Deploy history & rollback (`.zipgo-versions`)
Before it overwrites a site, `zipgo deploy` snapshots that site's **current own content** into a hidden `<site>/.zipgo-versions/<UTC-stamp>/` folder (`deploy.snapshot`/`snapshotScript`, run over SSH), keeping the newest `--keep N` (default `deploy.DefaultKeep` = 5; `--no-history`/`--keep 0` disables it). The snapshot rsync excludes `.zipgo-versions` and nested `*.` subdomain folders — the same content boundary the deploy mirror and `remote` sizing use — and is a no-op when there is no real prior deploy to capture (folder absent, or no `index.html`). `zipgo rollback <host> [version]` (`internal/remote/rollback.go`) lists (`Versions`/`parseVersions`, pure/table-tested) or restores a snapshot; restoring first snapshots the current live content (so a rollback is itself reversible), then rsyncs the chosen snapshot back over the live folder, again never touching history or child subdomain sites. The name `.zipgo-versions` is `sites.VersionsDirName` — **the single source of truth**; it is referenced by `internal/deploy` (snapshot + the `versionsExclude` rsync guard that keeps `--delete` from wiping history), `internal/builder` (added to the file_server `hide` list so snapshots are never served), and `internal/remote` (statScript prune + rollback). Discovery ignores it for free — `sites.Discover` skips dot-prefixed dirs, so a snapshot folder is never mistaken for a subdomain or content, and `HostFromRemote` rejects it (no trailing dot) so `ls` never lists it as a site.
...
### Internal packages
- **`internal/sites`** — recursively walks a domain folder; `Site` holds a leaf-first `Labels` chain (empty = apex). Helpers: `Host(root)`, `IsApex()`, `LocalhostPath(domain)`. Detects SPAs when `index.html` + one of `assets/`, `static/`, `_next/`, `dist/` is present (`DetectSPA`), and a custom 404 page when the folder holds a `404.html` (`DetectNotFound` → `Site.NotFoundPage`, kept as spelled on disk; `HasNotFoundPage()` gates it on the site being static — see below). `DetectSPA`/`DetectNotFound` are exported so `doctor`, which re-walks the tree itself, agrees with discovery instead of re-implementing the rules
...
A static site with a `404.html` in its folder serves it as the body of its 404s instead of Caddy's plain-text default — no config key, the file's presence *is* the config (`sites.NotFoundFileName`). `builder.fileHandler` attaches `errors` routes (`notFoundErrors`) to the site's subroute: a Caddy `subroute` runs its `errors.routes` when an inner handler returns an error, so a `file_server` 404 is matched (`{http.error.status_code} == 404` — only a 404, so a 403 or 500 is still Caddy's), rewritten to the page and re-served by a second `file_server` carrying `"status_code": "404"`. That `status_code` is the load-bearing bit: without it the page would go out as a **200 soft-404** and crawlers (and `curl -f`) would think the URL exists. The error routes live *inside* the subroute that `siteHandler` wraps in `basicAuthHandler`, so a protected site's 404 page stays behind its password.

`sites.HasNotFoundPage()` decides whether the route is emitted at all: only for a static site. An SPA already rewrites unknown paths to `/index.html` (a 200 — it never 404s), and `rewrite`/`redirect` sites never reach the file server, so in those folders the file would be dead weight — `doctor` warns rather than letting it look like it works. The page is resolved against the site's own root, so a subdomain does not inherit its parent's, and in localhost mode the rewrite is root-relative (not prefixed).
