...
CLI subcommands: `zipgo serve` (default), `zipgo deploy` (rsync a local dir to a remote zipgo host over SSH, creating the trailing-dot subdomain folder tree — see `internal/deploy`), `zipgo ls`/`zipgo info` (read-only SSH inspection of what's deployed on the remote target — see `internal/remote`), `zipgo doctor` (lint the local domains folder — see `internal/doctor`), `zipgo enable|disable|status` (systemd user service), `zipgo help`.
